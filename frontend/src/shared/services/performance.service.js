...
class PerformanceService {
  ...
  async getRecord(recordId) {
    const response = await apiService.get(
      `${API_PREFIX}/performance/${recordId}`
    );
  ...
  convertFormDataToBackendFormat(formData) {
    ...
    if (formData.salesEmployment) {
      dataJson.sales_employment = {
        ...
        export: {
          ...
          reporting_date:
            formData.salesEmployment.export?.reportingDate || null,
        },
        employment: {
          current_employees: {
            previous_year:
              formData.salesEmployment.employment?.currentEmployees
                ?.previousYear || null,
            reporting_date:
              formData.salesEmployment.employment?.currentEmployees
                ?.reportingDate || null,
          ...
          new_employees: {
            previous_year:
              formData.salesEmployment.employment?.newEmployees?.previousYear ||
              null,
            reporting_date:
              formData.salesEmployment.employment?.newEmployees
                ?.reportingDate || null,
          },
          total_employees: {
            previous_year:
              formData.salesEmployment.employment?.totalEmployees
                ?.previousYear || null,
            reporting_date:
              formData.salesEmployment.employment?.totalEmployees
                ?.reportingDate || null,
    ...
    if (
      ...
      dataJson.intellectual_property = formData.intellectualProperty.map(
        (item) => ({
          name: item.name,
          number: item.number,
          type: item.type,

...
export default function ThreadList() {
  ...
  const loadThreadDetail = useCallback(async (threadId) => {
    ...
    setThreads((prev) =>
      prev.map((t) => (t.id === threadId ? { ...t, adminUnreadCount: 0 } : t)),
    );
  }, []);
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      const newMessage = await messagesService.createThreadMessage(
        selectedThread.id,
        {
  ...
  const handleCloseThread = () =>
    selectedThread &&
    setCloseConfirm({ open: true, threadId: selectedThread.id });
  ...
  const handleReopenThread = async () => {
    ...
    setThreads((prev) =>
      prev.map((t) =>
        t.id === selectedThread.id ? { ...t, status: "open" } : t,
      ),
    );
    setMessageVariant("success");
  ...
  const getStatusBadge = (status) => {
    const map = {
      open: {
        variant: "success",
        label: t("admin.messages.thread.status.open"),
      },
      resolved: {
        variant: "info",
        label: t("admin.messages.thread.status.resolved"),
      },
      closed: {
        variant: "secondary",
        label: t("admin.messages.thread.status.closed"),
      },
    };
  ...
  const getCategoryBadge = (category) => {
    const map = {
      ...
      general: {
        variant: "secondary",
        label: t("admin.messages.category.general"),
      },
    };
  ...
  const formatDate = (d) =>
    d ? formatDateTime(d, "MM-dd HH:mm", i18n.language) : "";
  const totalUnread = threads.reduce(
    (sum, t) => sum + (t.adminUnreadCount || 0),
    0,
  );
  ...
  return (
    <div className="w-full">
      {message && (
        <Alert variant={messageVariant} className="mb-2">
          {message}
        </Alert>
      )}
          ...
          {["all", "open", "resolved", "closed"].map((key) => (
            ...
            >
              {key === "all"
                ...
                : {
                    open: t("admin.messages.thread.status.open"),
                    resolved: t("admin.messages.thread.status.resolved"),
                    closed: t("admin.messages.thread.status.closed"),
                  }[key] || key}
            </Button>
          ))}
        </div>
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-32 text-gray-400">
                  ...
                  <span className="text-sm">
                    {t("admin.messages.threads.empty")}
                  </span>
                </div>
              ) : (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span
                              className={`text-sm truncate ${thread.adminUnreadCount > 0 ? "font-semibold text-gray-900" : "text-gray-700"}`}
                            >
                              {thread.subject}
                            </span>
                          </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gradient-to-b from-gray-50/50 to-white">
                  {threadMessages.length === 0 ? (
                    ...
                    threadMessages.map((msg) => (
                      ...
                      >
                        <div
                          className={`flex items-end gap-2 max-w-[75%] ${msg.senderType === "admin" ? "flex-row-reverse" : ""}`}
                        >
                          <div
                          ...
                          >
                            {msg.senderType === "admin"
                              ? t("admin.messages.avatar.admin")
                              : (selectedThread.memberName ||
                                  t("admin.messages.avatar.member"))[0]}
                          </div>
                          <div
                          ...
                          >
                            ...
                            <div
                              className={`text-xs mt-1.5 ${msg.senderType === "admin" ? "text-blue-100" : "text-gray-400"}`}
                            >
                              {formatDate(msg.createdAt)}
                            </div>
                            {msg.attachments?.length > 0 && (
                              <div
                                className={`mt-2 pt-2 border-t ${msg.senderType === "admin" ? "border-blue-400/50" : "border-gray-100"} space-y-2`}
                              >
                ...
                {selectedThread.status === "open" ? (
                  <div className="p-3 border-t border-gray-100 bg-white">
                    <textarea
                      ...
                      placeholder={t(
                        "admin.messages.composer.contentPlaceholder",
                      )}
      ...
      <Modal
        ...
        onClose={() => setCloseConfirm({ open: false, threadId: null })}
        title={t("admin.messages.thread.confirmClose")}
        size="sm"
      >
        <div className="py-4">
          <p className="text-gray-600">
            {t("admin.messages.thread.closeWarning")}
          </p>
        </div>
        <ModalFooter>
          <Button
            variant="outline"
            onClick={() => setCloseConfirm({ open: false, threadId: null })}
          >
            {t("common.cancel")}
          </Button>
          <Button variant="primary" onClick={confirmCloseThread}>
            {t("admin.messages.thread.close")}
          </Button>
        </ModalFooter>
      </Modal>

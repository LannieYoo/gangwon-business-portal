/**
 * Logger Handler Utility
 * 
 * Utility functions for consistent logging across the application.
 * Moved from services/logger.service.js to utils/loggerHandler.js
 */
...
// Create a separate axios instance for logging to avoid circular dependency
const loggerApiClient = axios.create({
  baseURL: API_BASE_URL,
  timeout: 10000,
  headers: { 'Content-Type': 'application/json' },
});

// Add auth token to logger API requests
loggerApiClient.interceptors.request.use((config) => {
...
class LoggerService {
  ...
  async log(level, message, extra = {}) {
    ...
    loggerApiClient.post(`${API_PREFIX}/frontend/logs`, logEntry).catch((error) => {

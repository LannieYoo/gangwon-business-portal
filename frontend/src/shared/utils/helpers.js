...
export function formatPhoneNumber(value) {
  ...
  if (truncated.startsWith("02")) {
    ...
    if (truncated.length <= 9)
      return `${truncated.slice(0, 2)}-${truncated.slice(2, 5)}-${truncated.slice(5, 9)}`;
...
export function formatCurrencyCompact(value, options = {}) {
  ...
  const getUnit = (unit) => {
    if (t) {
      const currencyLabels = {
        won: t("common.currency.won"),
        krw: t("common.currency.krw"),
        man: t("common.currency.man"),
        cheon: t("common.currency.cheon"),
        unit: t("common.currency.unit"),
      };
      return currencyLabels[unit] || unit;
    }
    const units = {
      ko: { currency: "원", man: "만", cheon: "천", unit: "단위", krw: "원" },
      zh: { currency: "元", man: "万", cheon: "千", unit: "单位", krw: "元" },
    };
  ...
  if (absNum >= 10000) {
    ...
    const formatted = parseFloat(
      man.toFixed(decimals).replace(/\.?0+$/, ""),
    ).toLocaleString(locale);
    if (!showUnitLabel) return `${sign}${formatted}`;
    const unit = showCurrency
      ? `${getUnit("man")}${getUnit("krw")}`
      : getUnit("man");
  ...
  if (absNum >= 1000) {
    ...
    const formatted = parseFloat(
      cheon.toFixed(decimals).replace(/\.?0+$/, ""),
    ).toLocaleString(locale);
    if (!showUnitLabel) return `${sign}${formatted}`;
    const unit = showCurrency
      ? `${getUnit("cheon")}${getUnit("krw")}`
      : getUnit("cheon");
    return `${sign}${formatted} (${getUnit("unit")}: ${unit})`;
  }
  ...
  return unit
    ? `${sign}${formatted} (${getUnit("unit")}: ${unit})`
    : `${sign}${formatted}`;
}
...
export const MAX_IMAGE_SIZE = getEnvNumber(
  "VITE_MAX_IMAGE_SIZE",
  5 * 1024 * 1024,
);
export const MAX_DOCUMENT_SIZE = getEnvNumber(
  "VITE_MAX_DOCUMENT_SIZE",
  20 * 1024 * 1024,
);
export const MAX_FILE_SIZE = 20 * 1024 * 1024;

const ALLOWED_IMAGE_EXTENSIONS = getEnvArray("VITE_ALLOWED_IMAGE_EXTENSIONS", [
  "jpg",
  "jpeg",
  "png",
  "gif",
...
const ALLOWED_DOCUMENT_EXTENSIONS = getEnvArray(
  "VITE_ALLOWED_DOCUMENT_EXTENSIONS",
...
const imageMimeTypes = ALLOWED_IMAGE_EXTENSIONS.map(
  (ext) => extensionToMimeType[ext],
).filter(Boolean);
const documentMimeTypes = ALLOWED_DOCUMENT_EXTENSIONS.map(
  (ext) => extensionToMimeType[ext],
).filter(Boolean);

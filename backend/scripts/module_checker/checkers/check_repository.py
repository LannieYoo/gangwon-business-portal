...
class RepositoryChecker(
    ...
    def check_class(self, node: ast.ClassDef, file_path: str, content: str):
        """类级检查（按优先级顺序，发现问题即停止）。"""
        # Repository 特定检查
        self.check_repository_structure(node, file_path)

    def check_repository_structure(self, node: ast.ClassDef, file_path: str) -> None:
        """检查 Repository 结构规范。"""
        # 1. 检查是否实现了对应的 Repository 接口
        self.check_repository_interface_implementation(node, file_path)
        
        # 2. 检查依赖注入
        self.check_repository_dependency_injection(node, file_path)
        
        # 3. 检查数据访问逻辑
        self.check_repository_data_access(node, file_path)
    ...
    def check_class_structure(self, node: ast.ClassDef, file_path: str) -> None:
        """检查类结构。"""
        super().check_class_structure(node, file_path)
        self.check_repository_structure(node, file_path)


if __name__ == "__main__":
    run_checker(RepositoryChecker)
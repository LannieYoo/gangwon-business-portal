...
export default function NotificationHistoryPage(props) {
  ...
  const readOptions = useMemo(
    () => [
      ...
      { value: "read", label: t("support.read") },
    ],
    [t],
  );
  ...
  const getReadBadge = (isRead) => {
    return (
      <Badge variant={isRead ? "secondary" : "success"}>
        {isRead ? t("support.read") : t("notifications.unread")}
      </Badge>
    );
  };
  ...
  const getCategoryBadge = (subject, content) => {
    ...
    if (data) {
      ...
      const categoryMap = {
        ...
        project: {
          variant: "info",
          label: t("support.category.project"),
        },
        member: {
          variant: "primary",
          label: t("support.category.member"),
        },
        system: {
          variant: "secondary",
      ...
      <div className="w-full">
        <div className="mb-6 border-b border-gray-100 pb-4">
          <h1 className="text-2xl font-bold text-gray-900">
            {t("support.notificationHistory")}
          </h1>
          <p className="text-gray-600 mt-2 text-sm">
            {t('support.notificationHistory.description', '시스템 알림 및 공지 확인')}
          </p>
        </div>

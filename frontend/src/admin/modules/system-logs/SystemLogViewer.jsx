...
export default function SystemLogViewer({ initialFilter = null }) {
  ...
  const loadAllLogs = useCallback(async () => {
    try {
      ...
      const response = await logsService.listSystemLogs(params);
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex-1 min-w-[200px] max-w-md">
            <SearchInput
              ...
              placeholder={tl('systemLog.searchPlaceholder') || '搜索消息、模块...'}
            />
          </div>
          <Select
            value={filters.level}
      ...
      <div className="bg-white shadow-sm rounded-lg border border-gray-200">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-500">
            <p className="text-lg mb-2">{tl('systemLog.noLogs') || '暂无系统日志'}</p>
            <p className="text-sm text-gray-400">
              {totalCount === 0 ? '系统日志为空' : '当前搜索条件下没有匹配的日志'}
            </p>
          </div>
        ) : (
...
function SystemLogRow({ log, expanded, onToggle, tl, onReload }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirmModal.type === 'delete') {
        await logsService.deleteSystemLog(log.id);
      } else {
        await logsService.deleteSystemLogsByMessage(log.message);
      }
    ...
    <div className="hover:bg-gray-50">
      ...
      {expanded && (
        <div className="px-4 py-3 bg-gray-50 border-t border-gray-100">
          ...
          {/* 系统日志：只有 logger_name 且和 module 重复时不显示 extra_data */}
